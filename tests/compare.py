...
def create_test_plan():
    ...
    # Use same 98 spots as test_phantom_impt.py to ensure beam hits phantom
    n_spots = 98
...
def run_cuda_dose(dose, plan):
    ...
    result = np.array(dose.dose)
    ni, nj, nk = dose.size
    mid_i, mid_j, mid_k = ni//2, nj//2, nk//2
    print(f"\n[CUDA DEBUG] ========== CUDA Result ==========")
    print(f"[CUDA DEBUG] dose.size = {dose.size}")
    print(f"[CUDA DEBUG] dose.dose.shape = {result.shape}")
    print(f"[CUDA DEBUG] dose max = {result.max():.6f} at {np.unravel_index(result.argmax(), result.shape)}")
    print(f"[CUDA DEBUG] dose at center [{mid_i},{mid_j},{mid_k}] = {result[mid_i, mid_j, mid_k]:.6f}")
    print(f"[CUDA DEBUG] ===================================\n")
    return result
...
def main():
    ...
    dose_cuda.createCubePhantom(size=phantom_size)
    ...
    dose_jax1.createCubePhantom(size=phantom_size)
    ...
    dose_jax2.createCubePhantom(size=phantom_size)
