...
def _precompute_all_grids(ni: int, nj: int, nk: int,
    ...
    # CUDA mapping: i_cuda=dim2, j_cuda=dim1, k_cuda=dim0
    # So x comes from k_indices, y from j_indices, z from i_indices
...
def _raytrace_kernel(ni: int, nj: int, nk: int,
    ...
    def ray_step(step, wet_sum):
        ...
        # CUDA mapping: i=dim0→z, j=dim1→y, k=dim2→x
        tex_x = (ray_x + iso_x) / spacing  # k index
        tex_y = (ray_y + iso_y) / spacing  # j index
        tex_z = (ray_z + iso_z) / spacing  # i index
        
        within_bounds = (tex_x >= 0) & (tex_x < nk) & \
                       (tex_y >= 0) & (tex_y < nj) & \
                       (tex_z >= 0) & (tex_z < ni)
        
        # Stack as [i, j, k] = [z, y, x] for correct density indexing
...
def _smooth_wet_kernel(ni: int, nj: int, nk: int,
    ...
    def process_sample(carry, sample_idx):
        ...
        # CUDA mapping: i=dim0→z, j=dim1→y, k=dim2→x
        new_i = jnp.round((new_img_z + iso_z) / spacing).astype(jnp.int32)  # z→dim0
        new_j = jnp.round((new_img_y + iso_y) / spacing).astype(jnp.int32)  # y→dim1
        new_k = jnp.round((new_img_x + iso_x) / spacing).astype(jnp.int32)  # x→dim2
        
        # Check bounds (i indexes dim0, j indexes dim1, k indexes dim2)
...
def _pencil_beam_single_layer(ni: int, nj: int, nk: int, lut_len: int,
    ...
    # Return in same (ni, nj, nk) order as input grids
    return dose_array
...
def compute_raytrace(beam_params: BeamParams, dose_params: DoseParams,
    ...
    # Clip WET to non-negative values (CUDA behavior)
    return jnp.maximum(smoothed_wet, 0.0)
...
def compute_dose(beam_params: BeamParams, dose_params: DoseParams,
    ...
    # Initialize dose accumulator - same shape as input grids (ni, nj, nk)
...
def compute_impt_dose(beam_params: BeamParams, dose_params: DoseParams,
    ...
    if DEBUG:
        print(f"[JAX DEBUG] compute_impt_dose: ni={ni}, nj={nj}, nk={nk}")
    ...
    if DEBUG:
        print(f"[JAX DEBUG] grids.vox_head_x.shape = {grids.vox_head_x.shape}")
        print(f"[JAX DEBUG] grids.i_indices shape = {grids.i_indices.shape}")
        # Print some sample coordinate values
        mid_i, mid_j, mid_k = ni//2, nj//2, nk//2
        print(f"[JAX DEBUG] At center voxel [{mid_i},{mid_j},{mid_k}]:")
        print(f"[JAX DEBUG]   i_index={int(grids.i_indices[mid_i, mid_j, mid_k])}, j_index={int(grids.j_indices[mid_i, mid_j, mid_k])}, k_index={int(grids.k_indices[mid_i, mid_j, mid_k])}")
        print(f"[JAX DEBUG]   vox_xyz = ({float(grids.vox_xyz_x[mid_i, mid_j, mid_k]):.1f}, {float(grids.vox_xyz_y[mid_i, mid_j, mid_k]):.1f}, {float(grids.vox_xyz_z[mid_i, mid_j, mid_k]):.1f})")
        print(f"[JAX DEBUG]   vox_head = ({float(grids.vox_head_x[mid_i, mid_j, mid_k]):.1f}, {float(grids.vox_head_y[mid_i, mid_j, mid_k]):.1f}, {float(grids.vox_head_z[mid_i, mid_j, mid_k]):.1f})")
    
    # Compute WET using precomputed grids
    wet_array = compute_raytrace(beam_params, dose_params, density_array, grids)
    
    if DEBUG:
        print(f"[JAX DEBUG] wet_array.shape = {wet_array.shape}")
        wet_max_idx = np.unravel_index(np.array(wet_array).argmax(), wet_array.shape)
        print(f"[JAX DEBUG] wet_array max = {float(wet_array.max()):.6f} at {wet_max_idx}")
        # Print WET at center
        mid_i, mid_j, mid_k = ni//2, nj//2, nk//2
        print(f"[JAX DEBUG] WET at center [{mid_i},{mid_j},{mid_k}] = {float(wet_array[mid_i, mid_j, mid_k]):.4f}")
    ...
    if DEBUG:
        print(f"[JAX DEBUG] dose_array.shape from compute_dose = {dose_array.shape}")
...
def computeIMPTPlanJax(dose_grid, plan):
    ...
    if DEBUG:
        print(f"\n[JAX DEBUG] ========== computeIMPTPlanJax ==========")
        print(f"[JAX DEBUG] dose_grid.size = {dose_grid.size}")
        print(f"[JAX DEBUG] dose_grid.origin = {dose_grid.origin}")
        print(f"[JAX DEBUG] dose_grid.spacing = {dose_grid.spacing}")
        print(f"[JAX DEBUG] density_array.shape = {density_array.shape}")
    ...
    if DEBUG:
        print(f"[JAX DEBUG] dose_params: ni={dose_params.ni}, nj={dose_params.nj}, nk={dose_params.nk}")
    
    # Internal accumulator uses (ni, nj, nk) order - same as input grids
    output_shape = (dose_params.ni, dose_params.nj, dose_params.nk)
    total_dose = np.zeros(output_shape, dtype=np.float32)
    
    if DEBUG:
        print(f"[JAX DEBUG] output_shape (accumulator) = {output_shape}")
    ...
    for beam in plan.beam_list:
        ...
        if DEBUG:
            print(f"[JAX DEBUG] dose_array.shape from compute_impt_dose = {dose_array.shape}")
            print(f"[JAX DEBUG] dose_array max = {float(dose_array.max()):.6f} at {np.unravel_index(np.array(dose_array).argmax(), dose_array.shape)}")
    ...
    if DEBUG:
        print(f"[JAX DEBUG] total_dose.shape before return = {total_dose.shape}")
        print(f"[JAX DEBUG] total_dose max = {total_dose.max():.6f} at {np.unravel_index(total_dose.argmax(), total_dose.shape)}")
        print(f"[JAX DEBUG] ============================================\n")
    
    # Return in (ni, nj, nk) order - same as CUDA
    return total_dose
